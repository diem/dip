...
`invalid_x_request_sender_address`:
...
* The compliance key found from the onchain account is not a valid ED25519 public key.
...
##### Object Data / Logic Validation Error Codes
...
`missing_dependency`: one of dependency objects is not found by the id provided in the command object. For example, if the payment object `original_payment_reference_id` field is set, the receiver should return this error code if it could not find the data by the `original_payment_reference_id`.

`invalid_dependency`: one of dependency objects is found, but it is not valid for the command object. For example, the dependency payment object status is `aborted` instead of `ready_for_settlement`.
...
* Failed to acquire lock by for the command object by the `reference_id`.
