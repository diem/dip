...
Of particular note for off-chain APIs is the compliance key value.  This is the Ed25519 key with which a VASP signs travel rule statements which are verified on-chain and all off-chain requests and responses. During account creation, a VASP will set their compliance key which will be stored under the primary VASP account.
...
Both `X-REQUEST-ID` and `X-VERIFICATION-KEY-ADRRESS` are case insensitive.
...
| reference_id | str | Y | Unique reference ID of this payment on the payment initiator VASP (the VASP which originally created this payment Object). This value should be unique. This field is mandatory on payment creation and immutable after that. The field contains a subset of characters in the set [`A-Za-z0-9_-`] and has a maximum length of 64 characters. Should be a UUID according to [RFC4122](https://tools.ietf.org/html/rfc4122). |
