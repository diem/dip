...
The PaymentActorObject is extended to include the field `business_data` of type `BusinessDataObject`.
Field|Type|Required?|Description|
|-|-|-|-|
|business_data|BusinessDataObject|N|This element is required for some scenarios. It includes the details of the merchant to be displayed to the payer|
|
...
|-|-|-|-|
|reference_id|str|Y|Unique reference ID of this payment. This value should be globaly unique|
|status|StatusObject|Y|Initiator status (can be either the sender or receiver). For the abort command the status should be abort|
|
```
{
  "_ObjectType": "CommandRequestObject",
