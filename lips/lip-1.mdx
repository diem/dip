...
|---------|----------|-----------|
| v2      | current  | n/a       |
| v1      | e82a0eb8a9dd5d498fc89bf84565cc7adae3d8ef/lips/lip-1 | https://github.com/diem/lip/blob/e82a0eb8a9dd5d498fc89bf84565cc7adae3d8ef/lips/lip-1.mdx  |

V2 changes:

* Restricted to one actor to send command for mutating object.
* Removed Command `_writes` and `_reads` with object versioning.
* Changed cid, reference_id and X-REQUEST-ID to use 128 bit UUID according to [RFC4122](https://tools.ietf.org/html/rfc4122).
* Field `additional_kyc_data` is moved from KycDataObject to PaymentActorObject.
* Protocol URL is changed, removed sender/receiver's address in the URL path, added X-VERIFICATION-KEY-ADRRESS HTTP header for looking up JWS verification key.
* Removed 2 payment actor StatusEnum values: `needs_recipient_signature` and `pending_review`.
* Required to set sender's `kyc_data` for initial PaymentCommand.
* Required to set receiver's `kyc_data` and `recipient_signature` when receiver actor's status is set to `ready_for_settlement`.
...
All HTTP requests and responses must contain:

* A header `X-REQUEST-ID` with a unique UUID (according to [RFC4122](https://tools.ietf.org/html/rfc4122)) for the request, used for tracking requests and debugging. Responses must have the same string in the `X-REQUEST-ID` header value as the requests they correspond to.
* A header `X-VERIFICATION-KEY-ADRRESS` with the LIP-5 Diem address of the VASP signing the HTTP request or HTTP response. This header is used to retrieve the VASP verification key used to authenticate the request or response. The value must be the HTTP request sender’s VASP LIP-5 address used in the command, for example: the value should be sender/receiver actor address from PaymentObject in PaymentCommand.

Both `X-REQUEST-ID` and `X-VERIFICATION-KEY-ADRRESS` are case insensitive according to HTTP protocol [RFC2616](https://www.ietf.org/rfc/rfc2616.txt).
...
If VASP A fails to receive a response from VASP B, or in the case of specific protocol failure codes, it must re-send the request at some cadence until a response is received to ensure consistency.
...
* VASP A sends a request to VASP B failed with an unknown network error. VASP A retried the send request to VASP B.
...
* VASP B processed first request successfully and replied a success response, but VASP A didn’t receive it.
* VASP B respond a protocol error for the second request as it can’t acquire the object / data lock by reference id.
...
| cid | str    | Y     | A unique identifier for the Command. Should be a UUID according to [RFC4122](https://tools.ietf.org/html/rfc4122). |
...
| cid            | str      | N | The Command identifier to which this is a response. Should be a UUID according to [RFC4122](https://tools.ietf.org/html/rfc4122). |
...
```
{
    ...
    "command": {
	    ...
	    "payment": {
		    ...
		    "reference_id": "5b8403c986f53fe072301fe950d030cb",
		    "action": {
			    "amount": 100,
			    "currency": "USD",
			    "action": "charge",
...
| reference_id | str | Y | Unique reference ID of this payment on the payment initiator VASP (the VASP which originally created this payment Object). This value should be global unique. This field is mandatory on payment creation and immutable after that. We recommend use a 128 bits long UUID according to [RFC4122](https://tools.ietf.org/html/rfc4122). |
...
```
{
    ...
    "reference_id": "d4115900aad65d8141236b464f1315f5",
    "original_payment_reference_id": "5b8403c986f53fe072301fe950d030cb",
    "recipient_signature": "...",
...
* `soft_match` - Actor's KYC data resulted in a soft-match.
...
* SSOFTSEND (`need_kyc_data`, `soft_match`, `Sx`, `_`)
...
The data that contributes to the compliance recipient signature.
    ...
    reference_id (ascii): "bb991d8e3e6011eb8eaeacde48001122"
...
reference_id = "bb991d8e3e6011eb8eaeacde48001122"
