...
# Summary
This LIP describes the organization of Libra's proof of authority consensus model by which the Libra blockchain agrees upon ledger state. In a proof of authority consensus model, well-known participants leverage cryptographical digital signatures to agree upon a set of transactions and their output to advance the blockchain's state. The set of potential entities participating in consensus are known as Validators, while the active participants are known as the Validator Set. The Validator Set is part of a global on-chain configuration stored as part of the ledger state. Other examples of on-chain configuration include the list of registered currencies, publishing policies and Libra version. LibraRoot is the sole entity capable of adding or removing Validators and specifying the current Validator Set.

## LibraRoot account management
The Libra Payment Network ("LPN") and the Libra Association ("LA") manage the LibraRoot account. The LPN is a blockchain-backed payment platform and an infrastructure for stablecoins. Libra Networks (“Libra Networks” or “LN”) is a whole subsidiary of the LA which is in the process of applying for an LPN payment system license from the Swiss Financial Market Supervisory Authority (“FINMA”). Once granted, LN will be responsible for ensuring that LPN operates in compliance with the payment system license from FINMA (the "FINMA License").
...
The LA together with LPN, through Association Operation Services (the "AOS"), jointly manage the LibraRoot account that determines the composition of the Validator Set at any given time. Reasons for changing the Validator Set include new members joining the Association or existing members leaving it and actions required to comply with LN’s obligations under its FINMA License designed to guarantee the secure and performant operation of the platform.
...
* *Validator Config* is an on-chain configuration or Move resource that contains configuraiton information of a Validator Node. This includes a *consensus public key* and [network addresses](https://github.com/libra/libra/blob/master/specifications/network/network-address.md).
...
* *Validator Set* is an on-chain Move resource represented by a vector containing the active set of Validator Nodes. There exists a one-to-one relationship between a Validator Owner in the Validator Set and a Validator Node. The entries within the Validator Set consist of the Validator Owner's address as well as a copy of its Validator Config. Each Validator Owner address must appear only once in the Validator Set. Validator Nodes run according to the configuration defined in the Validator Set. The Validator Config in the Validator Set may be an older version of the Validator Config than is currently stored under Validator Owner's account. Validator Set changes in either the set of participants or the Validator Config result in a Reconfiguration.
* *Reconfiguration* defines a new epoch by emitting a `LibraConfig::NewEpochEvent`. This event is emitted as the result of any change in the global on-chain configuration. If a transaction causes an epoch change, that transaction is the last transaction committed in the current epoch — any subsequent transactions in that block or future blocks from that epoch will be ignored and available for later execution. The `LibraConfig::NewEpochEvent` marks the start of the new configuration computed as a result of the epoch ending transaction.
* *LibraRoot* is the sole entity capable of adding or removing Validators and specifying the current Validator Set. It may also submit an AdminScript.
...
LibraRoot (through the Association Operation Services, "AOS") creates Validator Owner and Validator Operator accounts. To get an account created, each Validator Owner or Operator needs to provide an authentication key and a name of their organization to LibraRoot. The authentication key is a 32 elements byte array, derived from the public verification key of Ed25519 or MultiEd25519 signature schemes.

LibraRoot creates Validator Owner's account by sending the `create_validator_account` transaction script. An account address of a Validator Owner is derived from their name. An Owner may ask the LibraRoot to manage the newly created account in which case for account creation an owner only needs to provide a name. In the case when LibraRoot maintains ownership of an Owner's account the auth key of that account is set to all-zeroes string and the transactions on behalf of such Owner are sent via AdminScripts.

LibraRoot creates Validator Operator's account by sending the `create_validator_account` transaction script. An account addresses of a Validator Operator is derived from their authentication key in the usual way.
