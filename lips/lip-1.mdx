...
* A header `X-VERIFICATION-KEY-ADRRESS` with the LIP-5 Libra address of the VASP signing the HTTP reqquest or HTTP response. This header is used to retrieve the VASP verification key used to authenticate the request or response. For HTTP requests, the header is set to the VASP parent address initiating the request. For HTTP response, the header is set to the parent VASP address of the VASP responding.

Headers shuld be upper-case as specified above.
