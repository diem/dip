...
**Shared Object**: All Objects contained within a Command are logically shared, meaning that each VASP has a copy of the Object and may create a new Command to modify it. For example, during the lifecycle of a `KycDataObject`, both VASPs will add information to it. The protocol does not support simultaneous updates to Objects. Rather, VASPs must take turns updating, each update incrementing the Object's Version (defined below).

**Version**: The state of an Object at a specific point in time. Upon creation an Object is assigned version 0. Every creation or mutation increments the Version  by one. To prevent VASPs overriding each other, each mutation command carries the previous Version and is conditionally to it.

**reference_id**: Every Object type contains a `reference_id` field which is a unique reference ID for the Object. The reference ID is always specified by the payment initiator VASP (the VASP which originally created the Object). This value should be unique. We recommend the hexadecimal representation of a fresh >16 bytes random byte-string as a way of creating such values. The field is Unicode utf-8 encoded.
...
As mentioned in [Terminology](#terminology), every Object type contains a `reference_id` field which is a unique reference ID for the Object. Each VASP also associates with an object its **latest version**, which is 0 upon creation and increases by one with every command on the object that succeeds.

Commands specify the list of objects and versions they expect to operate on in the `_reads` field. The `_reads` field contains a map of object `reference_id` to object versions.  A command is only allowed to read or mutate objects that are referred to within this map. Upon a command being successful all object versions are increased by one. A command failure leaves the object versions unchanged.
...
```
{
    ...
    "command": {
        ...
        "_reads": {
            "5b8403c986f53fe072301fe950d030cb": 0
        },
	    "payment": {
		    "sender": {
...
| _reads | JSON Object map |  Y | Must be an Object containing mappings of `reference_id` (str) to latest object Version (uint).  For a payment Command, only zero or one `_reads` values should be specified since payments are only dependent upon at most prior Version of an Object. A list with any other number of items results in a Command error.  If the list is empty this payment Command defines a new payment. If the list contains one item, then this Command updates the shared `PaymentObject`. |
```
{
    "_ObjectType": "PaymentCommand",
    "_reads": {
            "d4115900aad65d8141236b464f1315f5": 1
        },
    "payment": {
        PaymentObject(),
    }
...
| reference_id | str | Y | Unique reference ID of this payment on the payment initiator VASP (the VASP which originally created this payment Object). This value should be unique. This field is mandatory on payment creation and immutable after that. |
