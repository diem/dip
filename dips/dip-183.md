...
This DIP describes a protocol standard for pre-flight exchange using reference ID to form payment transactions.

# Abstract
Reference ID is used as a unique identifier for a payment transaction between two VASPs. 
We define a stanford for doing a pre-flight exchange of a reference ID between VASPs to agree on a reference ID for a transaction.
