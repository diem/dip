...
* `InitChargePayment` - Initializes a payment process of type `charge`. Triggering this command by the Wallet (Customer VASP) (Customer VASP) means the customer approved to proceed with the payment and that the Wallet (Customer VASP) (Customer VASP) is satisified with the business infromation provided. Successful response from the Merchant VASP means that the Merchant VASP is satisfied with the payer data provided by the Wallet (Customer VASP) (Customer VASP) 
* `AbortPayment` - Aborts a payment

| Command Type |Direction| Description | Request Data | Response Data |
|-|-|-|-|-|
|[GetPaymentInfo](#getpaymentinfo)|Customer VASP > Merchant VASP|By providing the object reference id the Wallet (Customer VASP) (Customer VASP) can retrieve the object information from the receiving VASP. In most cases, this would be the first command sent by the Wallet (Customer VASP) (Customer VASP). The Wallet (Customer VASP) is not expected to provide any payer data at this stage|Reference ID|Payment Details ; Business Data|
|[InitChargePayment](#initchargepayment)|Customer VASP > Merchant VASP|This functionality allows the Wallet (Customer VASP) to init the payment process based on the payment details. This is likely to occur after the customer approved the payment request and the Wallet (Customer VASP) (Customer VASP) verified the business data|Reference ID ; Payer Data|(optional) Recipient Signature|
...
|-|-|-|-|
|1|GetPaymentInfo|Customer VASP|The Wallet (Customer VASP) (Customer VASP) requests payment details using the reference id provided. The Merchant VASP will respond with the payment details and merchant data. The Wallet (Customer VASP) (Customer VASP) will display the payment details so the customer can approve or reject|
|2|InitChargePayment|Customer VASP|Following customer approval the Wallet (Customer VASP) (Customer VASP) initializes the payment process and sends the payer data|
|3|Put transaction on-chain|Customer VASP|Following a successful response from the Merchant VASP (to the InitChargePayment in the previous step), the Wallet (Customer VASP) (Customer VASP) must put a corresponding transaction on-chain and include the Reference ID as part of the metadata. See [appendix B](#appendix-b---include-reference-id-in-the-on-chain-transaction). If the payment is over the travel rule threshold, the Customer VASP must verify the recipient provided its signature as part of the InitChargePayment response|
...
`https://some-diem-Wallet (Customer VASP).com/pay?vasp_address=dm1pgxah7pxhzljvp3p4m9g0m3tm0qqqqqqqqqqqqqqgyftgh&reference_id=ad8d888a-1791-4b63-98e5-6f1d6ddb4411&redirect_url=http%3A%2F%2Fsomemerchantnname.com%2Fcallbackpage`
...
`diem://some-diem-Wallet (Customer VASP).com/pay?vasp_address=dm1pgxah7pxhzljvp3p4m9g0m3tm0qqqqqqqqqqqqqqgyftgh&reference_id=ad8d888a-1791-4b63-98e5-6f1d6ddb4411`
