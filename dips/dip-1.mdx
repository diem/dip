...
`invalid_dependency`: One of dependency objects is found, but it is not valid for the command object. For example, the status of the  dependency payment object found by `original_payment_reference_id` is `aborted` instead of `ready_for_settlement`.
