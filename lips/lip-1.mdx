...
Two VASPs participate in the off-chain protocol. They communicate through HTTP request and responses protected with TLS, and exchange messages that are signed to ensure authenticity and integrity. The goal of the off-chain protocol is for a pair of VASPs to jointly create a `PaymentObject` containing a reference-id. When a `PaymentObject` is **completed**, both VASPs have identical copies of it, and both and can map its reference-id to the object. A completed `PaymentObject` can be submitted by either VASP to the Blockchain to settle.
...
Each VASP exposes an HTTPS POST end point at `https://<hostname>:<port>/<protocol_version>/command`. The `protocol_version` is initially `v1` for this iteration of the off-chain APIs.

### HTTP Headers
All HTTP requests and responses must contain:

* A header `X-Request-ID` with a unique ID for the request, used for tracking requests and debugging. Responses must have the same string in the `X-Request-ID` header value as the requests they correspond to.
* A header `X-Sender-Address` with the LIP-5 Libra address of the VASP initating the HTTP reqquest. This header is used to retrieve the VASP vertifcation key used to verify the JWS signature of the payload included in the HTTP request, and to authenticate the request.
...
The contents of `payment` in a `PaymentCommand` either create or update a `PaymentObject`. The field `reference_id` must always be included in `payment` to determine which object is created or updated. In case of update there is an existing Object with the given `reference_id`, and the contents of the `payment` field update or add to the fields that already exist -- validation happens on the resulting, updated, object. This enables transfering only the updated fields within a `PaymentCommand`, rather than retransmitting the full state of the `PaymentObject` (which is also valid).
...
| reference_id | str | Y | Unique reference ID of this payment on the payment initiator VASP (the VASP which originally created this payment Object). This value should be unique. This field is mandatory on payment creation and immutable after that. The field contains a subset of characters in the set [`A-Za-z0-9_`] and has a maximum length of 64 characters. |
...
| metadata | list of str | N | Can be specified by the respective VASP to hold metadata that the sender/receiver VASP wishes to associate with this payment. It may be set to an empty list (i.e. `[]`). New string-typed entries can be appended at the end of the list, but not deleted.
...
The following is a concrete example of how to generate the Travel Rule dual attestation signable and the recipient signature using Libra Python Client SDK:
```
from dataclasses import dataclass
from pylibra import lcs
from pylibra import libra_types as libra
from pylibra import utils
from pylibra import serde_types as st
...

# Suffix of every signed dual attestation message
# (https://github.com/libra/libra/blob/8cf7dcb14b8a9c1049cf64c1a8fa299ffa550a6c/language/stdlib/modules/DualAttestation.move#L83)
DOMAIN_SEPARATOR = b"@@$$LIBRA_ATTEST$$@@"


metadata: libra.Metadata = libra.Metadata__TravelRuleMetadata(
...
# on-chain account address in hexadecimal (https://github.com/libra/lip/blob/master/lips/lip-5.md#terminology)
sender_libra_address_hex = "793251de1a61e9b4d1a17d13aa015e45
amount = 1_000_000

# Method1: use a dataclass

@dataclass
class Attestation:
    metadata: libra.Metadata
    sender_address: libra.AccountAddress
    amount: st.uint64

    def lcs_serialize(self) -> bytes:
        return lcs.serialize(self, Attestation)

attestation = Attestation(
    metadata=metadata,
    sender_address=utils.account_address(sender_libra_address_hex),
    amount=st.uint64(amount),
)
dual_attest_msg = attestation.lcs_serialize() + DOMAIN_SEPARATOR

# Method2: concat individually serialized bytes
...
dual_attest_bytearray: bytearray = bytearray()
dual_attest_bytearray.extend(lcs_metadata)
dual_attest_bytearray.extend(utils.account_address(sender_libra_address_hex).lcs_serialize())
dual_attest_bytearray.extend(lcs.serialize(st.uint64(amount), st.uint64))
dual_attest_bytearray.extend(DOMAIN_SEPARATOR)
dual_attest_msg = bytes(dual_attest_bytearray)
...
program = encode_peer_to_peer_with_metadata_script(
    ...
    100 /*amount*/,
    lcs_metadata,
    receiver_metadata_signature /* from off-chain API exchange */);
