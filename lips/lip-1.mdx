...
* Only allow one actor to send command for mutating object.
...
* additional_kyc_data field is moved from KycDataObject to PaymentActorObject.
...
* Removed payment actor StatusEnum: needs_recipient_signature and pending_review.
...
* A header `X-REQUEST-ID` with a unique UUID for the request, used for tracking requests and debugging. Responses must have the same string in the `X-REQUEST-ID` header value as the requests they correspond to.
