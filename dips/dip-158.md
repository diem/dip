...
This DIP, as described here, is an extension of the Off-Chain Protocol to support more advanced payment functionality - particularly eCommerce P2M payments. 
...
As soon as the funds are captured, i.e. the sender agreed to the payment terms, the sender VASP is expected to put a corresponding transaction on-chain and actually transfer the funds. 
...
Off-chain negotiations for Auth / Capture payment are divided into two steps accordingly. First, off-chain negotiations will result in funds being authorized, i.e. locked by the wallet. Later, a separate off-chain negotiation triggered by the receiving VASP will capture the funds.
...
* `authorized` - This means the actor has performed all required tasks and funds are ready to be locked by the wallet for a future capture
...
* A new field called `valid_until` added to the PaymentActionObject. This is a unix timestamp indicating the time period for which the payment can be authorized. Once this time has been reached, the payment cannot be authorized and funds cannot be locked
...
| valid_until | uint | N | Unix timestamp indicating the time period for which this authorization is valid.  Unix timestamp indicating the time period for which the payment can be authorized. Once this time has been reached, the payment cannot be authorized and funds cannot be locked. |
...
* `AuthCommand` - Notifies that payment funds are authorized (locked)for future capture
...
|NOTIFY_READY|Sender > Receiver|This functionality allows the wallet to indicate the payment is ready for settlement. The wallet is expected to put a transaction on-chain immediately before or after declaring it is ready|ReadyCommand|Reference ID||
|NOTIFY_AUTHORIZED|Sender > Receiver|This functionality allows the wallet to indicate funds are locked by for future capture. Relevant only for Auth / Capture payments|AuthCommand|Reference ID||
...
{
  "_ObjectType": "CommandRequestObject",
  "command_type": "AuthCommand",
  "cid": "3185027f-0574-6f55-2668-3a38fdb5de98",
  "command": {
    "_ObjectType": "AuthCommand",
    "reference_id": "4185027f-0574-6f55-2668-3a38fdb5de98"
    }  
}

```
#### AuthCommand Response
```
{
  "_ObjectType": "CommandResponseObject",
  "status": "success",
...
**Reminder:** The of exchanging a common payment identifier has been fulfilled

### Charge Sequence

| Step | Functionality | Triggered by | Description |
|------|-----------|-------------|------------|
|1|GET_INFO|Sender|The wallet requests payment details using the reference id provided (i.e. the common payment identifier). The receiver will respond with the payment details and merchant data. The wallet will display the payment details so the customer can approve or reject|
|2|INIT_PAYMENT|Sender|Following customer approval the wallet initializes the payment process and sends the payer data. For a successful flow, the receiver will respond with ready_for_settlement status|
|3|NOTIFY_READY|Sender|The wallet indicates the payment is ready for settlement. The wallet is expected to put a transaction on-chain immediately before or after this step|
| 
...
|------|-----------|-------------|------------|
|1|GET_INFO|Sender|The wallet requests payment details using the reference id provided (i.e. the common payment identifier). The receiver will respond with the payment details and merchant data. The wallet will display the payment details so the customer can approve or reject|
|2|INIT_PAYMENT|Sender|Following customer approval the wallet initializes the payment process and sends the payer data. For a successful flow, the receiver will respond with authorized status|
|3|NOTIFY_AUTHORIZED|Sender|The wallet indicates the funds are locked for a future capture|
| 
...
| Step |Functionality|Triggered by | Description |
|------|-----------|-------------|------------|
|1|CAPTURE_PAYMENT|Receiver|The receiver requests to capture the authorized funds|
|2|NOTIFY_READY|Sender|The wallet indicates the payment is ready for settlement. The wallet is expected to put a transaction on-chain immediately before or after this step|
|
...
|------|-----------|-------------|------------|
|1|GET_INFO|Sender|The wallet requests payment details using the reference id provided (i.e. the common payment identifier). The receiver will respond with the payment details and merchant data. The wallet will display the payment details so the customer can approve or reject|
|2|ABORT_PAYMENT|Sender|Customer rejected the payment request|
| 
...
| Step |State Code|Triggered by | Description |
|------|-----------|-------------|------------|
|1|VOID_PAYMENT|Receiver|Receiving VASP wants to void the payment. For example, the merchant voided the payment because the purchased products are out of stock|
|
...
|------|-----------|-------------|------------|
|1|GET_INFO|Sender|The wallet requests payment details using the reference id provided (i.e. the common payment identifier). The receiver will respond with the payment details and merchant data. The wallet will display the payment details so the customer can approve or reject|
|2|INIT_PAYMENT|Sender|Following customer approval the wallet initializes the payment process and sends the payer data.  Risk checks failed. Receiver will respond with abort status|
| 
...
|------|-----------|-------------|------------|
|1|GET_INFO|Sender|The wallet requests payment details using the reference id provided (i.e. the common payment identifier). The receiver will respond with the payment details and merchant data. The wallet will display the payment details so the customer can approve or reject|
|2|INIT_PAYMENT|Sender|Following customer approval the wallet initializes the payment process and sends the payer data. The receiver will respond with appropriate success status|
|3|ABORT_PAYMENT|Sender|Sender decided to abort. Can happen if the sender does not approve merchant data provided by the receiver|
| 
