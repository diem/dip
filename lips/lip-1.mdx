...
`https://<hostname>:<port>/<protocol_version>/<LocalVASPAddress>/<RemoteVASPAddress>/command`. `protocol_version` is initially 0 for this iteration of the off-chain APIs.  `LocalVASPAddress` and `RemoteVASPAddress` are the bech32 VASP addresses of the respective VASPs - with the subaddress set to 0's as per LIP 5. An example URL: http://127.0.0.1:7091/v1/lbr1pg9q5zs2pg9q5zs2pg9q5zs2pgyqqqqqqqqqqqqqqspa3m/lbr1pgfpyysjzgfpyysjzgfpyysjzggqqqqqqqqqqqqqxjt54z/command/
...
Sample generation of the `cid` can be done with something similar to:
```
...
```
'command_error' occurs in response to a command failing to be applied - for example, invalid `_reads` values.  `protocol_error` occurs in response to a failure related to the protocol.  Note: Additional information detailing all errors will be added.
